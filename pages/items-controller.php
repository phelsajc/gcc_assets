...
else if (isset($_POST['update']))
{
   ...
   $stmt = $pdo->prepare("UPDATE `items` SET `item_name`=?,`item_serial`=?,`item_model`=?,`item_qty`=? ,`item_price`=? ,`item_dop`=? ,`employee_id`=? ,`company_id`=?,`category_id`=?,`item_remarks`=?,`item_description`=?,`item_image`=? WHERE `item_id`=?");
...
{
            $id= $_GET['i_id'];       
            $query = $pdo->exec("DELETE FROM `items` WHERE `item_id` IN($id)");
            print $query;
}
