...
deleterow: function (rowid, commit) {
          var data = "delete=true&" + $.param({i_id: rowid});
                       $.ajax({
                            dataType: 'json',
                            url: 'items-controller.php',
                            cache: false,
        ...
        rendertoolbar: function (toolbar) {
                  var me = this;
                    ...
                    $("#deleteitem").on('click', function () {
                          var selectedrowindexes = $("#jq").jqxGrid('getselectedrowindexes');
                    var rowscount = $("#jq").jqxGrid('getdatainformation').rowscount;
                    // begin update. Stops the Grid's rendering.
                    $("#jq").jqxGrid('beginupdate');
                    ...
                    for (var m = 0; m < selectedrowindexes.length; m++) {
                        ...
                        if (selectedrowindex >= 0 && selectedrowindex < rowscount) {
                            var id = $("#jq").jqxGrid('getrowid', selectedrowindex);
                            $("#jq").jqxGrid('deleterow', id);
                        }
                    }
                    $("#jq").jqxGrid('endupdate');
                     });
            ...
            columns: [
                    ...
                    { text: 'Image', datafield: 'i_image', width: 50,cellsrenderer: imagerenderer},
